...
describe('Board Endpoints', () => {
    ...
    before(async () => {
        ...
        if (!boardId) {
            const boardRes = await request(server)
                .post(`/api/v1/tenants/${tenantId}/projects/${projectId}/boards`)
    ...
    it('should fetch the board by ID', async () => {
        const res = await request(server)
            .get(`/api/v1/tenants/${tenantId}/projects/${projectId}/boards/${boardId}`)
    ...
    it('should update the board', async () => {
        const res = await request(server)
            .patch(`/api/v1/tenants/${tenantId}/projects/${projectId}/boards/${boardId}`)
            .set('Authorization', `Bearer ${getToken(email)}`)
    ...
    it('should get all boards in the project', async () => {
        const res = await request(server)
            .get(`/api/v1/tenants/${tenantId}/projects/${projectId}/boards`)
            .set('Authorization', `Bearer ${getToken(email)}`);
    ...
    it('should delete the board', async () => {
        const res = await request(server)
            .delete(`/api/v1/tenants/${tenantId}/projects/${projectId}/boards/${boardId}`)
            .set('Authorization', `Bearer ${getToken(email)}`);
